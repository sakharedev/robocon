...
class DetectionAndDistance(Node):
    ...
    def __init__(self, dummy=True):
        ...
        self.subscription_right_frame = self.create_subscription(Image, '/camera_right/image', self.image_callback_right, 10)
        self.subscription_left_frame = self.create_subscription(Image, '/camera_left/image', self.image_callback_left, 10)
        ...
        self.get_logger().info('READY DetectionAndDistance')
    ...
    def image_callback_right(self, msg):
        self.get_logger().info('Received an right image.')
        self.current_right_image = msg

    def image_callback_left(self, msg):
        self.get_logger().info('Received an left image.')
        self.current_left_image = msg
    ...
    def detect_and_publish_red_ball(self):
        if self.current_image is not None:
            frame_right = self.bridge.imgmsg_to_cv2(self.current_right_image, desired_encoding='bgr8')
            frame_left = self.bridge.imgmsg_to_cv2(self.current_left_image, desired_encoding='bgr8')
            ...
            image_hsv_right = cv2.cvtColor(frame_right, cv2.COLOR_BGR2HSV)
            image_hsv_left = cv2.cvtColor(frame_left, cv2.COLOR_BGR2HSV) 

            # mask1 = cv2.inRange(image_hsv, lower_red1, upper_red1)
            mask_right = cv2.inRange(image_hsv_right, lower_red2, upper_red2)
            mask_left = cv2.inRange(image_hsv_left, lower_red2, upper_red2)

            cnts_right, _ = cv2.findContours(mask_right.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            cnts_left, _ = cv2.findContours(mask_left.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            
            c_num = 0
            radius = 10
            for i, c_left in enumerate(cnts_left):
                ((x_left, y_left), r_left) = cv2.minEnclosingCircle(c_left)
                if r_left > radius:
                    c_right = find_corresponding_ball(c_left, cnts_right)
                    ((x_right, y_right), r_right) = cv2.minEnclosingCircle(c_right)

                    disparity = x_left - x_right
                    distance = focal_length * baseline_distance / disparity

                    print('OK=', str(r_left), 'Distance=', str(distance))
                    c_num += 1
                    cv2.circle(frame_left, (int(x_left), int(y_left)), int(r_left), (0, 255, 0), 2)
                    cv2.putText(frame_left, '#{}: {:.2f}m'.format(c_num, distance), (int(x_left) - 10, int(y_left)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
                else:
                    print(r_left)


            image_msg = self.bridge.cv2_to_imgmsg(frame_right, encoding='bgr8')
            self.publish_image(image_msg)

            def find_corresponding_ball(c_left, cnts_right):
                ((x_left, y_left), r_left) = cv2.minEnclosingCircle(c_left)
                for c_right in cnts_right:
                    ((x_right, y_right), r_right) = cv2.minEnclosingCircle(c_right)
                    if abs(x_left - x_right) < 10:
                        return c_right
                return None
...
def main(args=None):
    ...
    node = DetectionAndDistance(dummy=False)
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
